 ...
 * -   pixels: intermediate buffer to avoid repeated mallocs across multiple
 * calls. Contents of this buffer do not matter. May be NULL, in which case it
 * is initialized. You must `free` it when you won't be calling this function
 * anymore.
 */
static void screenshot_ppm(const char *filename, unsigned int width,
  ...
  fprintf(f, "P3\n%d %d\n%d\n", width, height, 255);
  *pixels =
      realloc(*pixels, format_nchannels * sizeof(GLubyte) * width * height);
  glReadPixels(0, 0, width, height, GL_RGB, GL_UNSIGNED_BYTE, *pixels);
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      cur = format_nchannels * ((height - i - 1) * width + j);
      fprintf(f, "%3d %3d %3d ", (*pixels)[cur], (*pixels)[cur + 1],
              (*pixels)[cur + 2]);
    }
    fprintf(f, "\n");
  }
...
#if LIBPNG
/* Adapted from
 * https://github.com/cirosantilli/cpp-cheat/blob/19044698f91fefa9cb75328c44f7a487d336b541/png/open_manipulate_write.c
 */
...
int main(int argc, char **argv) {
  ...
  /* Work. */
  if (offscreen) {
    /* TODO: if we use anything smaller than the window, it only renders a
     * smaller version of things. */
